...
- **InpDepth**: минимальное число баров удержания экстремума до фиксации разворота MF‑pivot (пивот подтверждается, только если от последнего кандидата прошло не меньше `InpDepth` баров и цена отошла не меньше `InpDeviation`).
- **InpDeviation**: минимальное ценовое отклонение от последнего экстремума, необходимое для смены направления.
- **ATR‑адаптация порога**: если `AtrDeviationK > 0`, фактический порог = `max(InpDeviation * _Point, AtrDeviationK * ATR(tf,14))`. На высокой волатильности пивоты реже «сыпятся», на низкой — не «засыпают».
- **Без репейнта**: учитываются только подтверждённые экстремумы; незавершённые кандидаты ZigZag игнорируются. Текущий (незакрытый) бар всегда исключён. Кэш pivot не затирается нулями — пока новый экстремум не подтверждён, используется предыдущий.
...
- Подтверждение пробоя H1: ретест ищется строго ПОСЛЕ закрытия часа (`fromTime = iTime(H1,1) + PeriodSeconds(H1)`), а не внутри него.
...
- ❌ **Жёсткий выход (hard):** по умолчанию — пересечение pivot H1 ценой в противоположном направлении
...
```
- **Exit_H1 (дефолт)**: выход по пересечению `pivotH1`.
- **Exit_EntryTF**: выход по пересечению `pivotM5` (ТФ входа).
- **Exit_Nearest**: выход по ближайшему из `pivotM5/M15/H1` с фильтром минимальной дистанции `ExitNearestAtrK * ATR(M15)`; расстояние считается от цены входа (закрытой).
- **Exit_SoftHard**: soft‑exit — пересечение `pivotM15` (сигнал в `EarlyExitBuffer`) без сброса состояния; hard‑exit — пересечение `pivotH1` со сбросом.
