...
- **Без репейнта**: ZigZag/MF‑pivot исключает текущий (незакрытый) бар; расчёт ведётся только по закрытым.
...
- Если медиана < `FlatMedianThreshold` (дефолт 1.20) — флет: разрешены только ранние входы.
```mql5
