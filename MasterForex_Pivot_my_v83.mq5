//+------------------------------------------------------------------+
//| MasterForex-V MultiTF Indicator v8.3                             |
...
//+------------------------------------------------------------------+
//| Ensure that history for the timeframe is synchronized             |
//+------------------------------------------------------------------+
bool EnsureHistory(string symbol, ENUM_TIMEFRAMES tf, int attempts=40)
  {
   for(int i=0; i<attempts; i++)
     {
      if(SeriesInfoInteger(symbol, tf, SERIES_SYNCHRONIZED))
         return true;
      Bars(symbol, tf);            // request loading of data
      Sleep(100);
     }
   return SeriesInfoInteger(symbol, tf, SERIES_SYNCHRONIZED);
  }
...
int OnInit()
  {
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "MasterForex-V MultiTF v8.3");
...
double GetLastPivot(string symbol, ENUM_TIMEFRAMES tf, int &handle)
  {
   if(handle == INVALID_HANDLE)
     {
      ...
      // fallback: allow specifying folder without filename
      if(handle == INVALID_HANDLE)
         handle = iCustom(symbol, tf, ZigZagPath + "\\ZigZag", 12, 5, 3);
      if(handle == INVALID_HANDLE)
        {
   ...
   if(!EnsureHistory(symbol, tf))
     {
      Print("History not ready for tf=", tf);
      return 0.0;
     }

   int attempts = 0;
   ResetLastError();
   while(BarsCalculated(handle) <= 0 && attempts < 100)
     {
      Sleep(50);
      attempts++;
     }
   if(BarsCalculated(handle) <= 0)
     {
      ...
      Print("ZigZag data not ready tf=", tf, " err=", err);
   ...
   double zzBuffer[];
   ResetLastError();
   int copied = CopyBuffer(handle, 0, 0, 200, zzBuffer);
   if(copied <= 0)
     {
      int err = GetLastError();
   ...
   for(int i=1; i<copied; i++)      // skip current bar
      if(zzBuffer[i] != 0.0)
         return zzBuffer[i];
   return 0.0;
  }
