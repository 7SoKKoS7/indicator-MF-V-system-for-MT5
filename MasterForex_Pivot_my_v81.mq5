//+------------------------------------------------------------------+
//| MasterForex-V MultiTF Indicator v8.1                             |
...
int OnInit()
  {
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "MasterForex-V MultiTF v8.1");
...
double GetLastPivot(string symbol, ENUM_TIMEFRAMES tf, int &handle)
  {
   ...
   // ensure required history is loaded for the requested timeframe
   ...
   if(bars <= 0)
     {
      ...
      Print("BarsCalculated ZigZag tf=", tf, " result=", bars, " err=", err);
      return 0.0;
     }
