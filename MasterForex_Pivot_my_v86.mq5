...
double GetLastPivot(ENUM_TIMEFRAMES tf)
  {
   ...
   double zigzag[], highMap[], lowMap[];
   ArrayResize(zigzag,200);
   ArrayResize(highMap,200);
   ArrayResize(lowMap,200);
   ...
   for(int shift=start; shift<200; shift++)
     {
      int idxMin=ArrayMinimum(low, shift, depth);
      double val=low[idxMin];
      if(val!=lastLow)
        {
         lastLow=val;
      ...
      int idxMax=ArrayMaximum(high, shift, depth);
      val=high[idxMax];
      if(val!=lastHigh)
        {
         lastHigh=val;
